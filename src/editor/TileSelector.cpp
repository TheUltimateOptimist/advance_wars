...
TileSelector::TileSelector(QWidget* parent) : QScrollArea(parent)
{
    ...
    layout->addLayout(
        creatSectionLayout(tilebox, 1, 17, 21, 19, 20, 18, 26, 25, 27, 24, 29, 22, 28, 23));
...
void TileSelector::sectionLayout(
    QGridLayout*& layout, int usedIdCounter, QWidget* parent, T id, Rest... ids)
{
    TileButton* button = new TileButton(id, parent);
    layout->addWidget(button, usedIdCounter / 3, usedIdCounter % 3);
    usedIdCounter++;
